...
from app.schemas.auth import AuthenticatedUser, UserReturn
...
@router.post('/verify', status_code=200, response_model=UserReturn)
def verify(db: Annotated[Session, Depends(get_db)], user_id: Annotated[int, Depends(get_current_user)]):
  return auth.verify(db, user_id)